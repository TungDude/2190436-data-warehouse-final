...
def load(
    ...
    # Type 1 attributes"). The scd2_merge above only writes new versions —
    ...
    #
    # Without this propagation, fact rows that resolve to a v1 dim row see
    # community_area_name=NULL and the dashboards display "Unknown" for
    # every row. The UPDATE below copies the current-version name onto all
    # historical versions of the same community_area so SCD1 semantics
    # hold across the SCD2 version chain.
